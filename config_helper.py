    ...
    def get_val(self, so_str: str, isint: bool = False):
        ...
        if section in self.__config_obj.sections():
            ...
            if option in options:
                ...
                logger.debug('Option中不存在{}'.format(option))
                raise IniNotFindOptionError()
        else:
            logger.debug('Section中不存在{}'.format(section))
...
#     i = IniFileHelper()
#     logger.debug(i.get_val('mangoDB_config.port', isint=True))
